...
1. QLoRA is the Superior SFT Method
Base vs. FFT vs. QLoRA: The Qwen3-0.6B base model achieved 50.6% accuracy. Standard Full Fine-Tuning (FFT) on the PQA-A dataset improved this slightly to 57.2%. However, QLoRA significantly outperformed both, reaching 68.2% accuracy.

  Why? FFT on the artificial dataset likely caused catastrophic forgetting of the base model's pre-trained medical knowledge. QLoRA's rank constraint acted as a regularizer, allowing the model to learn the QA format without overwriting core reasoning capabilities.

2. DPO Maintains Task Performance
Alignment without Degradation: Applying Direct Preference Optimization (DPO)—using preferences generated by the much larger II-Medical-8B teacher—resulted in 68.0% accuracy.

  Significance: A McNemar test confirmed there is no statistically significant difference (p=0.625) between QLoRA and DPO accuracy. This proves we can successfully align the 0.6B model to an expert model's reasoning style without degrading its hard classification accuracy.

3. Naïve Knowledge Distillation (KD) Fails
The KD Collapse: Full-parameter Knowledge Distillation on the PQA-A dataset (using II-Medical-8B pseudo-labels) severely degraded performance, dropping accuracy to 41.3% (worse than the base model).

  The "No" Class Problem: Looking at the Per-Class F1 Heatmap, KD completely collapsed on the challenging "no" class (F1 dropped to 16.2%). Pure sequence-level KD on a noisy dataset overwrote the strong QLoRA initialization with the teacher's idiosyncratic mistakes.
